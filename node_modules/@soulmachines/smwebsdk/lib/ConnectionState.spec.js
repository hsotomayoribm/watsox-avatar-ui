...
describe('Connection State', function () {
    ...
    it('resets the connection state to be the first step when reset() is called', function () {
        connectionState.setConnectionState(ConnectionStateTypes_1.ConnectionStateTypes.Connected);
    ...
    describe('when there are five steps', function () {
        ...
        describe('when we are on step one', function () {
            beforeEach(function () {
                connectionState.setConnectionState(ConnectionStateTypes_1.ConnectionStateTypes.SearchingForDigitalPerson);
            });
            it('calls onConnectionStateUpdated with the step name', function () {
                expect(callbackFn).toBeCalledWith(expect.objectContaining({
                    name: ConnectionStateTypes_1.ConnectionStateTypes.SearchingForDigitalPerson,
                }));
            });
        ...
        describe('when we are on step two', function () {
            beforeEach(function () {
                connectionState.setConnectionState(ConnectionStateTypes_1.ConnectionStateTypes.DownloadingAssets);
            });
            it('calls onConnectionStateUpdated with the step name', function () {
                expect(callbackFn).toBeCalledWith(expect.objectContaining({
                    name: ConnectionStateTypes_1.ConnectionStateTypes.DownloadingAssets,
                }));
            });
            it('calls onConnectionStateUpdated with the current step', function () {
                expect(callbackFn).toBeCalledWith(expect.objectContaining({
        ...
        describe('when we are on step three', function () {
            beforeEach(function () {
                connectionState.setConnectionState(ConnectionStateTypes_1.ConnectionStateTypes.ConnectingToDigitalPerson);
            });
            it('calls onConnectionStateUpdated with the step name', function () {
                expect(callbackFn).toBeCalledWith(expect.objectContaining({
                    name: ConnectionStateTypes_1.ConnectionStateTypes.ConnectingToDigitalPerson,
                }));
            });
            it('calls onConnectionStateUpdated with the current step', function () {
                expect(callbackFn).toBeCalledWith(expect.objectContaining({
        ...
        describe('when we are on step four', function () {
            ...
            it('calls onConnectionStateUpdated with the step name', function () {
                expect(callbackFn).toBeCalledWith(expect.objectContaining({
                    name: ConnectionStateTypes_1.ConnectionStateTypes.Connected,
                }));
            });
            it('calls onConnectionStateUpdated with the current step', function () {
                expect(callbackFn).toBeCalledWith(expect.objectContaining({
        ...
        describe('when we are disconnected from connected', function () {
            beforeEach(function () {
                connectionState.setConnectionState(ConnectionStateTypes_1.ConnectionStateTypes.Connected);
                connectionState.setConnectionState(ConnectionStateTypes_1.ConnectionStateTypes.Disconnected);
            });
            it('calls onConnectionStateUpdated with the step name', function () {
                expect(callbackFn).toBeCalledWith(expect.objectContaining({
                    name: ConnectionStateTypes_1.ConnectionStateTypes.Disconnected,
